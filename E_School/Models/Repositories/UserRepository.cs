...
namespace MvcInternetShop.Models.Repositories
{
    ...
    //{
    //    private MvcShopEntities db = null;

    //    public UserRepository()     //سازنده کلاس
    //    {
    ...
    //            db.Entry(entity).State = System.Data.Entity.EntityState.Modified;       //اگر تغییر هم نکرده باشه بازم در دیتابیس ذخیره میشه
    //            if (autoSave)
    //                return Convert.ToBoolean(db.SaveChanges());
    //            else
    //                return false;
    ...
    //        try
    //        {
    //            //var entity = db.Users.Select(p => p.Id == id).Single();
